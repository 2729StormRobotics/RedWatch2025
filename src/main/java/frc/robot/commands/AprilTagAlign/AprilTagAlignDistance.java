...
public class AprilTagAlignDistance extends Command implements VisionIO {
  ...
  public AprilTagAlignDistance(Drive drivetrain,
  ...
  public void execute() {
    ...
    if (results.hasTargets()) {
      ...
      targetRange = PhotonUtils.calculateDistanceToTargetMeters(
          0.5, // Camera Height *CHANGE*
          0.305, // Target Height *CHANGE perhaps
          Units.degreesToRadians(0), // Camera Pitch *CHANGED*
          Units.degreesToRadians(target.getPitch()));
          
    SmartDashboard.putNumber("id number", target.getFiducialId());
      // FIND ANGLE FROM ID
      switch(target.getFiducialId()) {
        // BLUE SIDE TARGETS 18 is closest to DS
        case 22:
          targetAngle = -30;
          seeingTargets = true;
          break;
    ...
    // the tag.
    if ((safeZone) && !(Math.abs(targetYaw) < yawThreshold)) {
    	lateralSpeed = -0.25 * targetYaw * Constants.VisionConstants.kPTurn * DriveConstants.kMaxSpeedMetersPerSecond;
     }
     else {
	lateralSpeed = 0;
     }
    if (!(safeZone) || (safeZone && aligned)) {
	forward = forwardMultiplier * targetRange * Constants.VisionConstants.kPStrafe * DriveConstants.kMaxSpeedMetersPerSecond;
    }
    else {
	forward = 0;
    }
