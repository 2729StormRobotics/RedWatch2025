 ...
 */
public class AprilTagAlignTest extends Command implements VisionIO {
  ...
  public void execute() {
    ...
    switch (currentState) {
      ...
      case APPROACHING:
        if (results.hasTargets() && results.getBestTarget() != null) {
          ...
          if (stateTimer.get() > 0.25) { // Transition to ALIGNING after a delay
    ...
    double rotationSpeed = MathUtil.clamp(pidController.calculate(currentAngle), -MAX_ROTATION_SPEED,
        MAX_ROTATION_SPEED);
